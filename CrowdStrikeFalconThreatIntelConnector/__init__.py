...
def convert_to_stix(indicators: list):
    ...
    for indicator in indicators:
        labels = {"actors": indicator['actors'], "reports": indicator['reports'], "targets": indicator['targets'], "vulnerabilities": indicator['vulnerabilities'],"relations": indicator['relations']}
        try:
            for x in indicator["labels"]:
                List = x['name'].split("/")
                Value = []
                Field = List[0]
